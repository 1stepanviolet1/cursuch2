...
Mirror::__draw_y_mirror(png::Image &_image) const
{
    ...
    std::int64_t _m_x = _l_u.x();
    std::int64_t _m_y = _r_d.y();
    png::Color temp_color;

    for (std::int64_t _y = _l_u.y(); _y <= (_l_u.y() + _r_d.y()) / 2; _y++)
    {
        for (std::int64_t _x = _l_u.x(); _x <= _r_d.x(); _x++)
        {
...
Mirror::__draw_x_mirror(png::Image &_image) const
{
    ...
    std::int64_t _m_x = _r_d.x();
    std::int64_t _m_y = _l_u.y();
    png::Color temp_color;

    for (std::int64_t _y = _l_u.y(); _y <= _r_d.y(); _y++)
    {
        for (std::int64_t _x = _l_u.x(); _x <= (_l_u.x() + _r_d.x()) / 2; _x++)
        {
            temp_color = _image.pixel({_x, _y});
            _image.pixel({_x, _y}, png::Color(_image.pixel({_m_x, _m_y})));
            _image.pixel({_m_x, _m_y}, temp_color);
